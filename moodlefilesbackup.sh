...
_data_zipfile="${BACKUPPATH}/${_sitename}_data_backup_${_now}.tar.gz"
...
if [[ ${DEBUG} = "TRUE" ]] ; then
	...
	cat ~/mdl_conf.sh
	echo || echo
	echo "Note: backup has not occurred, omit debug parameter to execute backup"
	echo || echo
else	
	###################
	# start the backup
	# backup the moodle files
	echo -e "Starting backup of ${MOODLEPATH} directory of site ${_sitename} timestamped at datetime $_now"
	tar -vczf ${_files_zipfile} ${MOODLEPATH}

	echo || echo

	# backup moodledata
	echo -e "Starting backup of ${dataroot} Moodle data directory of site ${_sitename} timestamped at datetime $_now"
	tar -vczf ${_data_zipfile} ${dataroot}

	echo || echo

	# end of backup
	echo -e "Ending backup of site ${_sitename} timestamped at datetime $_now"
